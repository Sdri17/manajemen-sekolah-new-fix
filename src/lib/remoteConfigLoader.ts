...
export async function fetchRemoteFirebaseConfig(options?: { forceRefresh?: boolean }): Promise<FirebaseConfigType> {
  ...
  // Priority 1: Custom config stored in localStorage or Cookie (User explicitly saved a custom database)
...
export function getRemoteFirebaseConfig(): FirebaseConfigType {
  ...
  if (typeof window !== 'undefined') {
    const cookieCustom = getCookieCustomConfig();
    if (cookieCustom && cookieCustom.projectId && cookieCustom.apiKey) {
    ...
    if (localCustom) {
      try {
        ...
        if (parsed && parsed.projectId && parsed.apiKey) {
          return parsed as FirebaseConfigType;
        }
      } catch (_e) {}
    }
